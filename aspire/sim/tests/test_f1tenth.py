...
@pytest.mark.integration
def test_controller_executes_through_code_execution_env() -> None:
    pytest.importorskip("f110_gym")
    config = DictLoader.load("env_configs/f1tenth/levine_privileged.yaml")
    env = instantiate(config["env"])
    try:
        env.reset(seed=101, options={"start_pose": [0.0, 0.0, 0.0]})
        code = """
obs = get_observation()
obs = drive(0.0, 0.5)
